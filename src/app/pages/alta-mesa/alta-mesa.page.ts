...
export class AltaMesaPage implements OnInit{
  ...
  readonly tipos: readonly TipoMesa[] = ['VIP', 'estándar', 'movilidad_reducida'];
  ...
  formAltaMesa: FormGroup = this.fb.group({
    ...
    tipo: this.fb.control<TipoMesa | null>(null, [Validators.required, this.tipoValido()]),
  ...
  private tipoValido(): ValidatorFn {
    return (c: AbstractControl) =>
      this.tipos.includes(c.value as TipoMesa) ? null : { tipoInvalido: true };
  }
  ...
  async enviar() {
    ...
    try {
      ...
      if (existe) { this.err = 'La mesa ya existe'; this.loading = false; return; }

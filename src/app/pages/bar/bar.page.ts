...
export class BarPage implements OnInit, OnDestroy {
  ...
  async cargar() {
    const { data } = await supabase.from("bar_pedidos").select("*").eq("estado", "pendiente").order("creado_en", { ascending: false });
    this.pedidos = (data ?? []) as any[];
  }

  async terminar(id: string) {
    await supabase.from("bar_pedidos").update({ estado: "terminado", terminado_en: new Date().toISOString() }).eq("id", id);

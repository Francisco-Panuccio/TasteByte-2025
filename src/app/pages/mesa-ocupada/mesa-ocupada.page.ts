...
export class MesaOcupadaPage implements OnInit, OnDestroy, AfterViewInit {
  ...
  private sentPushNuevoPedido = new Set<string>();

  private accelSub?: PluginListenerHandle;
  private orientSub?: PluginListenerHandle;

  private lastHoldStart: Record<'L' | 'R' | 'F' | 'B', number> = {
    L: 0,
    R: 0,
    F: 0,
    B: 0,
  ...
  async ngOnInit() {
    ...
    try {
      ...
      if (!this.mesaId || Number.isNaN(this.mesaId)) {
  ...
  ngAfterViewInit(): void {
    this.presentingEl = document.querySelector(
      'ion-router-outlet'
    ) as HTMLElement;
    const setVars = () => {
  ...
  private async startMotion(): Promise<void> {
    this.stopMotion();
    this.orientSub = await Motion.addListener(
      'orientation',
      (e: OrientationListenerEvent) => {
        const beta = e.beta ?? 0;
        ...
        if (gamma <= -this.TILT_TH)
          this.markHold('L', now, () => this.siguienteFoto());
        else this.lastHoldStart.L = 0;
        if (gamma >= this.TILT_TH)
          this.markHold('R', now, () => this.anteriorFoto());
        else this.lastHoldStart.R = 0;

        const sign = gamma > this.TILT_TH ? 1 : gamma < -this.TILT_TH ? -1 : 0;
        if (Math.abs(gamma) <= this.ROLL_GUARD) {
          if (beta >= this.TILT_TH)
            this.markHold('F', now, () => this.siguienteProducto());
          else this.lastHoldStart.F = 0;

          if (beta <= -this.TILT_TH)
            this.markHold('B', now, () => this.anteriorProducto());
    ...
    this.accelSub = await Motion.addListener('accel', (ev) => {
      ...
      if (s !== this.lastShakeSignX) {
        ...
        while (
          this.togglesX.length &&
          now - this.togglesX[0] > this.SHAKE_WIN_MS
        )
          this.togglesX.shift();
  ...
  private stopMotion(): void {
    ...
    this.orientSub?.remove();
    this.orientSub = undefined;
    this.togglesX.length = 0;
  ...
  private getCards(): HTMLElement[] {
    ...
    return g
      ? (Array.from(g.querySelectorAll(':scope > .card')) as HTMLElement[])
      : [];
  }
  ...
  private getCarritoItems(): HTMLElement[] {
    return Array.from(
      document.querySelectorAll('.carrito-lista .carrito-item')
    ) as HTMLElement[];
  }
  ...
  private siguienteProducto(): void {
    if (this.carritoFlag) {
      const i = this.computeCarritoIndex();
      this.scrollToCarrito(i + 1);
    } else {
      const i = this.computeCardIndex();
      this.scrollToCard(i + 1);
    }
  }
  ...
  private resetAlPrimerProducto(): void {
    if (this.carritoFlag) {
      ...
      const track = first
        ? (first.querySelector('.track') as HTMLElement | null)
        : null;
  ...
  private applyEstado(raw: any) {
    ...
    this.estadoPedido = (ok as readonly string[]).includes(norm)
      ? (norm as Estado)
      : null;
    this.updateBanner();
  }
  ...
  private async detectarYPoblarPedido(): Promise<void> {
    ...
    if (this.clienteEmail) {
      try {
        const activoSvc = await this.pedidos.getPedidoActivo({
          mesaId: this.mesaId,
  ...
  incByPlato(p: Plato) {
    if (this.submitting || this.pedidoBloqueado) {
      return;
    }
    const id = p.id!;
      ...
      for (const p of list) {
        ...
        if (q > 0) {
          arr.push({
            ...
            precioUnit: p.precio,
            cantidad: q,
            duracionMin: p.tiempo_elaboracion_min,
          });
          total += p.precio * q;
      ...
      for (const b of list) {
        ...
        if (q > 0) {
          ...
          arr.push({
            productoId: id,
            tipo: 'bebida',
            nombre: (b as any).nombre as string,
            precioUnit: precio,
            cantidad: q,
            duracionMin: dur,
          });
  ...
  private buildPlatoYBebida(): void {
    const mk = (tipos: Array<'plato' | 'postre' | 'bebida'>) => {
      ...
      const total = Number(
        items.reduce((a, b) => a + b.subtotal, 0).toFixed(2)
      );
    ...
    this.plato = {
      titulo: `Plato • Mesa ${mesaNumero}`,
      total: c.total,
      cantidad: c.cantidad,
      items: c.items,
    };
    this.bebidaObj = {
      titulo: `Bebidas • Mesa ${mesaNumero}`,
      total: b.total,
      cantidad: b.cantidad,
      items: b.items,
    };
    ...
    try {
      ...
      if (existente) {
        if (existente.estado !== 'rechazado') {
          ...
          await this.pedidos.reemplazarItems(
            existente.id,
            this.itemsSel,
            this.total,
            this.etaMin,
        ...
        if (rej?.id) {
          ...
          try {
            const upd: any = {};
            if (this.anonimoId) {
              upd.anonimo_id = this.anonimoId;
            }
            if (this.usuarioId !== null) {
              upd.usuario_id = this.usuarioId;
            }
            if (this.clienteId !== null) {
              upd.cliente_id = this.clienteId;
            }
      ...
      this.unsubEstado?.();
      this.unsubEstado = this.pedidos.onEstadoPedido(
        pedidoId!,
        async (estado: any) => {
          this.zone.run(async () => {
            ...
            if (this.estadoPedido === 'aceptado') {
                ...
                if (tok) {
                  await this.push.send(
                    ...
                    'Tu pedido fue rechazado. Modifícalo y reenvíalo.',
                    {
                      ...
                      pedidoId,
                      mesaId: this.mesaId,
                      mesaNumero: this.mesa?.numero ?? null,
                    }
                  );
                }
              } catch {}
              ...
              (
                await this.toast.create({
                  message:
                    'Su pedido fue rechazado, por favor modifíquelo correctamente y reenvíelo.',
                  position: 'top',
                  cssClass: 'toast',
                  duration: 1200,
                })
      ...
      try {
        if (pedidoId && !this.sentPushNuevoPedido.has(pedidoId)) {
          this.sentPushNuevoPedido.add(pedidoId);

          const mesaNumero = this.mesa?.numero ?? this.mesaId ?? 'NN';
          const title = 'Nuevo pedido recibido';
          const body = `Mesa ${mesaNumero}: nuevo pedido del cliente.`;
          const data = {
            tipo: 'nuevo_pedido',
            mesaId: this.mesaId,
            pedidoId,
          ...
          const mozoTokens = (mozos ?? [])
            .map((m: any) => m.token as string)
            .filter(Boolean);

          const uniqueTokens = Array.from(new Set(mozoTokens));

          if (uniqueTokens.length) {
            await this.push.send(uniqueTokens, title, body, data);
            console.log('📨 Push enviada a mozos:', uniqueTokens.length);
          }
        }
      } catch (err) {
        console.warn('⚠️ Error enviando push al mozo:', err);
      }
  ...
  private buildEncuestaQuery(): any {
    const q: any = {};
    if (this.anonimoId) {
      q.anonimoId = this.anonimoId;
    }
    if (this.usuarioId !== null) {
      q.usuarioId = this.usuarioId;
    }
    if (this.clienteId !== null) {
      q.clienteId = this.clienteId;
    }
    q.tienePermiso = true;
    q.qrValido = true;
    return q;
  ...
  private gotoEncuestasEspera(): void {
    this.router.navigate(['/encuestas-espera'], {
      queryParams: this.buildEncuestaQuery(),
    });
  }

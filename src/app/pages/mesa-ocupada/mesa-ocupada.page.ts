...
export class MesaOcupadaPage implements OnInit, OnDestroy, AfterViewInit {
  ...
  async ngOnInit() {
    ...
    try {
      ...
      this.deliveryFlag = qp.get('delivery') === 'true';
      ...
      if (!this.deliveryFlag) {
        ...
        this.deliveryAddress = qp.get('address') ?? '';
        const latStr = qp.get('lat');
        const lngStr = qp.get('lng');
        this.deliveryLat = latStr && latStr !== '' ? Number(latStr) : undefined;
        this.deliveryLng = lngStr && lngStr !== '' ? Number(lngStr) : undefined;
      }
      ...
      if (!this.deliveryFlag && this.estadoPedido === 'aceptado') {
  ...
  private scrollToSlide(track: HTMLElement | null, index: number): void {
    ...
    try { track.scrollTo({ left: target.offsetLeft, behavior: 'smooth' }); } catch { track.scrollLeft = target.offsetLeft; }
  }
  ...
  private resetAlPrimerProducto(): void {
    if (this.carritoFlag) this.scrollToCarrito(0);
    else {
      this.scrollToCard(0);
  ...
  private applyIdentityFilters(q: any): any {
    ...
    else if (this.anonimoId) q = q.eq('anonimo_id', this.anonimoId);
    else if (this.usuarioId !== null) q = q.eq('usuario_id', this.usuarioId);
    else if (this.clienteId !== null) q = q.eq('cliente_id', this.clienteId);
    return q;
  }
  ...
  private async buscarPedidoActivoDb(): Promise<PedidoRow | null> {
    ...
    if (this.deliveryFlag) {
      ...
      q = q.eq('tipo', 'mesa').eq('mesa_id', this.mesaId);
    }
  ...
  private async buscarUltimoRechazadoDb(): Promise<PedidoRow | null> {
    ...
    if (this.deliveryFlag) q = q.eq('tipo', 'delivery').is('mesa_id', null);
    else {
      if (!this.mesaId) return null;
      q = q.eq('tipo', 'mesa').eq('mesa_id', this.mesaId);
    }
    ...
    if (this.clienteEmail) {
      try {
        const activoSvc = await this.pedidos.getPedidoActivo({
          mesaId: this.deliveryFlag ? undefined : this.mesaId,
          delivery: this.deliveryFlag,
  ...
  private buildPlatoYBebida(): void {
    ...
    const ctx = this.deliveryFlag
      ? (this.deliveryAddress ? `Delivery • ${this.deliveryAddress}` : 'Delivery')
      : `Mesa ${this.mesa?.numero ?? 'NN'}`;

    this.plato = { titulo: `Plato • ${ctx}`, total: c.total, cantidad: c.cantidad, items: c.items };
    this.bebidaObj = { titulo: `Bebidas • ${ctx}`, total: b.total, cantidad: b.cantidad, items: b.items };
  }

  async terminarPedido() {
    try {
      if (!this.itemsSel.length) return;
      if (!this.deliveryFlag && !this.mesaId) throw new Error('Mesa inválida.');
      ...
      if (!existente && this.clienteEmail) {
        try {
          ...
          if (svc) {
            existente = { id: svc.id as string, estado: (svc as any).estado, mesa_id: this.mesaId ?? null } as PedidoRow;
          }
        } catch { }
      }
      ...
      if (existente) {
        ...
        pedidoId = await this.pedidos.crearPedido(
          this.deliveryFlag ? null : this.mesaId!,
        ...
        try {
          ...
          if (Object.keys(upd).length) await supabase.from('pedidos').update(upd).eq('id', pedidoId);
        } catch { }
      }
        ...
        this.zone.run(async () => {
          ...
          if (this.estadoPedido === 'aceptado') {
            ...
            (await this.toast.create({
              message: 'Pedido rechazado. Modifícalo y reenvíalo.',
              position: 'top',
              cssClass: 'toast',
              duration: 1200,
            })).present();
      ...
      try {
        ...
        const roles = this.deliveryFlag ? ['dueno', 'supervisor'] : ['mozo'];
        ...
        if (tokens.length) await this.push.send(tokens, title, body, data);
      } catch { }
  ...
  volver() { this.gotoEncuestasEspera(); }
}
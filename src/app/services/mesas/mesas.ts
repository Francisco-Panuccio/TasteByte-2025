...
export class Mesas {
  ...
  async getByNumeroDeMesa(numero: number): Promise<Mesa | null> {
    const { data, error } = await supabase.from(this.table).select("*").eq("numero", numero).single();
    if (error && error.code !== "PGRST116") throw error;
    return data ?? null;
  }
  ...
  async create(payload: Mesa): Promise<Mesa> {
    const { data, error } = await supabase
      .from(this.table)
      .insert({
        numero: (payload as any).numero,
        capacidad: (payload as any).capacidad,
        tipo: (payload as any).tipo,
        foto_url: (payload as any).foto_url
      })
      .select("*")
      .single();
    if (error) throw error;
  ...
  async uploadPhotoBlob(fileName: string, blob: Blob): Promise<string> {
    ...
    const { error: upErr } = await supabase.storage.from("mesas").upload(path, blob, { contentType: blob.type || "image/jpeg", upsert: true });
  ...
  async liberarMesa(mesaId: number): Promise<void> {
    const { error: errAsig } = await supabase.from("asignaciones_mesa").delete().eq("mesa_id", mesaId);
    if (errAsig) throw errAsig;
    const { error: errLista } = await supabase.from("lista_espera").delete().eq("mesa_id", mesaId);
    if (errLista) throw errLista;
    const { error: errPedidos } = await supabase.from("pedidos").delete().eq("mesa_id", mesaId);
